...
; Fred/FRONT
;
;Usage 1: ADPCM_Encode.exe -d Input.bin Output.wav
...
int main(int argc, char* argv[])
{
	...
	printf("NeoGeo PCM En-/Decoder\n----------------------\n");
	if (argc < 4)
	{
		printf("Usage: ADPCM_Encode.exe -option InputFile OutputFile\n");
		printf("-option can be:\n");
		printf("\t-d for decode (bin -> wav)\n");
		printf("\t-e for encode (wav -> bin)\n");
		printf("\n");
		printf("Wave Output is 16-bit, Mono, 44.1 KHz\n");
		printf("Wave Input must be 16-bit, Mono, Samplerate is ignored\n");
		return 1;
	}
